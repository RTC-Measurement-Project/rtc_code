...
def process_single_file(
    ...
    stage2_filtered_streams_count = {
        "TCP": 0,
        "UDP": 0,
    }
    stage1_filtered_packets_count = {
        "TCP": 0,
        "UDP": 0,
    }
    stage2_filtered_packets_count = {
        "TCP": 0,
        "UDP": 0,
    }

    # STAGE 1: Apply time-based filters

    # Apply precall filter
    s1_results_precall = time_based_filter(
        streams,
        start_time_dt,
        "precall",
        offset,
    ...
    stage1_precall_filtered_streams, _, _, _ = s1_results_precall
    for stream_type, stream_dict in stage1_precall_filtered_streams.items():
    ...
    stage1_postcall_filtered_streams, _, _, _ = s1_results_postcall
    for stream_type, stream_dict in stage1_postcall_filtered_streams.items():
        stage1_filtered_streams_count[stream_type] += len(stream_dict)
        # Count packets in filtered streams
        for stream_id in stream_dict:
            if stream_id in streams[stream_type]:
    ...
    # Apply precall filter
    s2_results_precall = time_based_filter(
        streams,
        start_time_dt,
        "precall",
        offset,
    ...
    stage2_precall_filtered_streams, _, _, _ = s2_results_precall
    for stream_type, stream_dict in stage2_precall_filtered_streams.items():
    ...
    stage2_postcall_filtered_streams, _, _, _ = s2_results_postcall
    for stream_type, stream_dict in stage2_postcall_filtered_streams.items():
        stage2_filtered_streams_count[stream_type] += len(stream_dict)
        # Count packets in filtered streams
        for stream_id in stream_dict:
            if stream_id in streams[stream_type]:
    ...
    update_json_attribute(pcap_info_file, "Stage 1 Filtered Streams Count", stage1_filtered_streams_count)
    update_json_attribute(pcap_info_file, "Stage 2 Filtered Streams Count", stage2_filtered_streams_count)
    update_json_attribute(pcap_info_file, "Stage 1 Filtered Packets Count", stage1_filtered_packets_count)
    update_json_attribute(pcap_info_file, "Stage 2 Filtered Packets Count", stage2_filtered_packets_count)
