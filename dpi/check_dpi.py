...
protocol = "rtcp" # can be "rtp" or "stun" or "rtcp" or "classicstun"

debug = False

start_packet_index = 1979
end_packet_index = 1979
suspecious_flow = ('206.247.29.194', '10.75.43.10', 8801, 53080, 16779266, 113)
...
def validate_rtp_info_list(message_info_list, packet_count):
    ...
    for flow_id, messages in flow_dict.items():
        ...
        messages_sorted = sorted(messages, key=lambda x: (x['seq_num'], x['packet_index']))

        clusters = []
        current_cluster = [messages_sorted[0]]
        for i in range(1, len(messages_sorted)):
            prev = messages_sorted[i - 1]
            curr = messages_sorted[i]
            seq_diff = int(curr['seq_num']) - int(prev['seq_num'])
            ts_diff = int(curr['timestamp']) - int(prev['timestamp'])
    ...
    if 1:
        ...
        for flow_id, messages in debug_flow_group.items():
            ...
            for pkt in messages:
                print(f"  Packet {pkt['packet_index']} (chopped {pkt['chopped_bytes']} bytes), SSRC: {pkt['ssrc']}, Seq Num: {pkt['seq_num']}, Version: {pkt['version']}, Padding: {pkt['padding']}, Extension: {pkt['extension']}, CC: {pkt['cc']}, Marker: {pkt['marker']}, Payload Type: {pkt['payload_type']}, Timestamp: {pkt['timestamp']}")
...
def validate_stun_info_list(message_info_list, packet_count):
    ...
    if 1:
        ...
        for message_info in message_info_list:
            ...
            # print(f"  Packet {message_info['packet_index']} (chopped {message_info['chopped_bytes']} bytes), "
            #       f"msg_type: {message_info['msg_type']}, msg_length: {message_info['msg_length']}, "
            #       f"transaction_id: {message_info['transaction_id']}")
...
def validate_classic_stun_info_list(message_info_list, packet_count):
    if 1:
        ...
        for message_info in message_info_list:
            print(f"  Packet {message_info['packet_index']} (chopped {message_info['chopped_bytes']} bytes), message length: {message_info['message_length']}")
...
def validate_rtcp_info_list(message_info_list, packet_count):
    ...
    if 1:
        ...
        for message_info in filtered_message_info_list:
            print(f"  Packet {message_info['packet_index']} (chopped {message_info['chopped_bytes']} bytes), ssrc: {message_info['ssrc']}, packet_type: {message_info['packet_type']}")
    return filtered_message_info_list
