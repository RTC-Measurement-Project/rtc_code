...
if __name__ == '__main__':
    ...
    if not os.path.exists(accuracy_report_dir):
        os.makedirs(accuracy_report_dir)
    ...
    for dpi_file in dpi_files:
        ...
        baseline_file_name = dpi_file_name.replace('_dpi_detection.txt', '_part1_messages.txt')
        baseline_file =f"{heuristic_baselines_dir}/{baseline_file_name}"
        report_file_name = dpi_file_name.replace('_dpi_detection.txt', '_accuracy_report.txt')
        report_file =f"{accuracy_report_dir}/{report_file_name}"
